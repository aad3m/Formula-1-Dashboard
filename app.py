...
season_options = [str(y) for y in range(this_year, 2009, -1)]  # 2025..2010
...
# ---------- Data (now season-aware) ----------
completed_round = get_completed_round(season)
schedule_df = get_schedule(season)
drivers_df = get_driver_standings(season)
teams_df = get_constructor_standings(season)
results_df = get_all_results_up_to(season, completed_round)
...
# UI
render_header(f"{APP_TITLE} — {season}", APP_CAPTION)
